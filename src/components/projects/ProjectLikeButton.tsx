...
const ProjectLikeButton = ({ projectId, initialLikesCount = 0 }: ProjectLikeButtonProps) => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    const fetchLikesCount = async () => {
      try {
        const { data, error } = await supabase.rpc('count_project_likes', { 
  ...
  // Après une connexion réussie, fermer la boîte de dialogue et liker le projet
  const handleAuthSuccess = () => {
    setAuthDialogOpen(false);
    
    // Ajouter un petit délai pour s'assurer que le contexte d'authentification est à jour
    setTimeout(() => {
      if (pendingLike) {
        handleLikeToggle();
    ...
    <>
      ...
      <Dialog open={authDialogOpen} onOpenChange={setAuthDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Connectez-vous pour soutenir ce projet</DialogTitle>
          </DialogHeader>
